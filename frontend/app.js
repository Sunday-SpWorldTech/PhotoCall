...
const SOCKET_URL = (import.meta.env.VITE_SOCKET_URL || API).replace(/\/$/, '');
...
files.onchange=e=>{[...e.target.files].filter(f=>/^image\/(jpeg|png|webp)$/.test(f.type)).forEach(f=>photos.push({url:URL.createObjectURL(f),name:f.name}));selected=Math.max(0,photos.length-1);galleryRender();loadPhoto();say(`${photos.length} photo${photos.length===1?'':'s'} ready.`)};
$('#flip').onclick=()=>{mirrored=!mirrored;drawAvatar()};$('#reset').onclick=()=>{mirrored=false;selected=0;loadPhoto();galleryRender()};
...
$('#shareSignal').onclick=async()=>{const text=inviteText();try{if(navigator.share){await navigator.share({title:'PhotoCall call',text});say('Call invite ready to share. Select Signal if it is available.');}else{await navigator.clipboard.writeText(text);say('PhotoCall invite copied. Open Signal and paste it into your chat.');}}catch(e){if(e.name!=='AbortError')say('Unable to open sharing. The invite was not sent.');}};
