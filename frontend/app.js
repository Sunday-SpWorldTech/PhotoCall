...
$('#shareSignal').onclick=async()=>{const text=inviteText();try{if(navigator.share){await navigator.share({title:'PhotoCall call',text});say('Signal share opened. PhotoCall avatar stays in PhotoCall; Signal mobile does not expose an external camera API.');}else{await navigator.clipboard.writeText(text);say('PhotoCall invite copied. Open Signal and paste it into your chat.');}}catch(e){if(e.name!=='AbortError')say('Unable to open sharing. The invite was not sent.');}};
